...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(1, 18, kernel_size=9, stride=1, padding=0, bias=True),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.layer2 = nn.Sequential(
            nn.Conv2d(18, 36, kernel_size=5, stride=1, padding=0, bias=True),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.drop_out = nn.Dropout()
        self.fc1 = nn.Linear(3 * 3 * 36, 100)
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        for i, (images, labels) in enumerate(train_loader):
            ...
            if (i + 1) % 100 == 0:
                print('Epoch {}, Mini batch [{}/{}], Score: {:.1f}%'
                .format(epoch, (i + 1), (len(train_loader)), (cci / tni) * 100.0))
                tni, cci = 0, 0
